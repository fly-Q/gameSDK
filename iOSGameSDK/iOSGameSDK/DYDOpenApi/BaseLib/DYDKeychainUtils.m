...
static NSString *DYDKeychainUtilsErrorDomain = @"SFHFKeychainUtilsErrorDomain";
...
@implementation DYDKeychainUtils

#if __IPHONE_OS_VERSION_MIN_REQUIRED < 30000 && TARGET_IPHONE_SIMULATOR

+ (NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
    if (!username || !serviceName) {
        *error = [NSError errorWithDomain: DYDKeychainUtilsErrorDomain code: -2000 userInfo: nil];
        return nil;
    }
    ...
    if (status != noErr) {
        *error = [NSError errorWithDomain: DYDKeychainUtilsErrorDomain code: status userInfo: nil];
        return nil;
    }
...
+ (NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
    ...
    if (status != noErr) {
        if (status == errSecItemNotFound) {
            ...
            if (error != nil) {
                *error = [NSError errorWithDomain: DYDKeychainUtilsErrorDomain code: -1999 userInfo: nil];
            }
        }
        else {
...
+ (BOOL) storeUsername: (NSString *) username andPassword: (NSString *) password forServiceName: (NSString *) serviceName updateExisting: (BOOL) updateExisting error: (NSError **) error
{
    ...
    NSString *existingPassword = [DYDKeychainUtils getPasswordForUsername: username andServiceName: serviceName error:&getError];
